...
<script>
	...
	$(function() {
		...
		//初始化列表
		queryList();
		var str = "${paper.pauditopinion}";
		str = str.trim();
		if(null !=str&&"null"!=str&&"fbok"!=str&&""!=str){
			alert("试卷被退回："+str);
		}
		var pstatus = "${paper.pstatus}";
		if("3" == pstatus){
			$("#baocun").hide();
			$("#tijiao").hide();
		}
	...
	function importpaper(){
		...
		$.post("<c:url value='/paper/importpaper.html' />",{"paper.pid":pid,"qids":qids,}, function(response) {
			if (response.success) {
				if(confirm("提交审核成功！"))
				{
				}
...
<div class="headbar" >
		...
		<div class="operating" style="position:relative; overflow:visible;border-top: 0px;">
			 <a href="javascript:void(0)" onclick="viewCheckQues();"><button class="operating_btn" type="button" title="预览已选中试题"><span class="view">预览</span></button></a>
     	     <a id="baocun" href="javascript:void(0)" onclick="savePaper();"><button class="operating_btn" type="button" title="试卷保存"><span class="send">保存</span></button></a>
     	     <a id="tijiao" href="javascript:void(0)" onclick="importpaper();"><button class="operating_btn" type="button" title="试卷保存"><span class="import">提交</span></button></a>
     	     <a style="display:none;" href="javascript:void(0)" onclick="passexamine();"><button class="operating_btn" type="button" title="通过审核"><span class="grade">通过审核</span></button></a>
     	     <a style="display:none;" href="javascript:void(0)" onclick="unpassexamine();"><button class="operating_btn" type="button" title="退回"><span class="return">退回</span></button></a>
