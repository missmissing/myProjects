	...
	$(function() {
		...
		$("#update").bind("click", function() {
			...
			if ($("#questionForm").validationEngine("validate")) {
				$.post("<c:url value='/question/update.html' />",{ jsonStr:serializeObjJson()}, function(response) {
					if (response.success) {
						if(confirm("修改试题成功！"))
						{
						}
			...
			<form id="questionForm" method="post" accept-charset="utf-8" enctype="multipart/form-data">
				...
				<table class="form_table">
					...
					<tr >
						<th></th>
						<td>
						<span style="color: #FF0000" id="auditopinion">${question.qauditopinion }</span>
						</td>
					</tr>
					<tr >
						<th></th>
						<td>
							<input type="button" class="submit" id="update" value="保存修改"></input>
