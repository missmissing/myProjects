...
<script type="text/javascript">
	...
	/**导入试题*/
	function quesInput(){
		parent.openIframePage("导入试题","<c:url value='/quesinput/quesinput.html' />");
	}
/*************************popup选择树  begin************************************************************************/
...
function editQuestion(id){
	var url = "<c:url value='/question/edit4examine.html?qid="+encodeURIComponent(id)+"' />";
...
function queryList(){
	...
	var url = "<c:url value='/question/query4examine.html' />";
	$.ajax({
		url : url ,
		...
		success : function(pageJson) {
			...
			for(var i=0;i<tableJson.length;i++){
				...
				tbodyDiv+="<td>";
				if(status != "发布"){
					tbodyDiv+="<p class='betn_edit' title='审核' onclick='editQuestion(\""+tableJson[i].qid+"\")' />";
					tbodyDiv+="<p class='betn_del' title='删除' onclick='deleteQustion(\""+tableJson[i].qid+"\",this)'/>";
				}	
				else{
					tbodyDiv+="<p class='betn_down' title='下线' onclick='downQuestion(\""+tableJson[i].qid+"\")' />";
				}
				tbodyDiv+="</td>";
				tbodyDiv+="</tr>";
			}
...
//下线
function downQuestion(id) {
		$.post("<c:url value='/question/downQuestion.html' />",{"id":id}, function(response) {
			if (response.success) {
...
<div class="headbar">
		...
		<div class="operating" style="position:relative; overflow:visible ">
     	     ...
     	     <a style="display:none;" href="javascript:void(0)" onclick="quesInput();"><button class="operating_btn" type="button"><span class="export">导入</span></button></a>
