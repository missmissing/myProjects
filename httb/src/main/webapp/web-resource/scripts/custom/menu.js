jQuery(function(){
	...
	//文字滚动显示
	$("#tips :not(:first)").css("display","none");
	...
	//点击收缩左栏
	$('#info_bar').css('text-align', 'left');
	$('#info_bar').append("<span id='zhankai' style='display:inline-block;margin-top:5px;margin-left:15px;cursor:pointer'>收缩</span>");
	$('#zhankai').toggle(function() {
		$("ul.submenu>li>span").next().css("display","none");
		$("ul.submenu>li>span").addClass("selected");
		$('#zhankai').html('展开');
	}, function() {
		$("ul.submenu>li>span").next().css("display","");
		$("ul.submenu>li>span").removeClass("selected");
		$('#zhankai').html('收缩');
	});
